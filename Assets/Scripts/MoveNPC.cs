    ...
    private void OnAnimatorMove()
    {
        switch (mode)
        {
            case Mode.AgentControlsPosition:
                //Debug.Log("MoveNPC using tranfor.position = agent.nextPosition");// the 1st one is a null ref - the rest r ok - timing?
                if (agent) transform.position = agent.nextPosition;  //"if (agent)" resolved the null ref 
                break;
            case Mode.AnimatorControlsPosition:
                if (agent)  // untested "if" see above comment of resolving null ref 
                {
